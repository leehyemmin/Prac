							...
							<form id='modifyForm' role="form" action="/admin/product/modify" method="post" enctype="multipart/form-data">
								<div class="box-body">
									<div class="form-group">
										...
										<select class="form-control" id="mainCategory" name="ctg_prt_code" style="width:30%; margin-right:10px; display: inline-block;" >
										  <option value="default">1차 카테고리 선택</option>
										  <c:forEach items="${categoryList}" var="cate">
										  	<option value='<c:out value="${cate.ctg_code }" />' <c:out value="${cate.ctg_code == ProductVO.ctg_code ? 'selected' : '' }" />>
										  		<c:out value="${cate.ctg_name }" />
										  	</option>
										  </c:forEach>
										</select>
										
										<select class="form-control" id="subCategory" name="ctg_code" style="width: 30%; display: inline-block;">
										 	<option value="default">2차 카테고리 선택</option>
										 	<c:forEach items="${subCategoryList}" var="subCate">
										  	<option value='<c:out value="${subCate.ctg_code }" />' <c:out value="${subCate.ctg_code == ProductVO.ctg_code ? 'selected' : '' }" />>
										  		<c:out value="${subCate.ctg_name }" />
										  	</option>
										  </c:forEach>
										</select>
									</div>
									...
									<div class="form-group">
										...
										<input type="text" id="prod_name" name="prod_name" value='<c:out value="${productVO.prod_name }"/>' class='form-control' >
										<input type="hidden" id="prod_num" name="prod_num" value='<c:out value="${productVO.prod_num }"/>' class="form-control">
									</div>
									<div class="form-group">
										<label for="exampleInputCompany">Company</label> 
										<input type="text" id="prod_company" name="prod_company" value='<c:out value="${productVO.prod_company }"/>' class="form-control">
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<input style="width:40%; margin-right:10px; display: inline-block;"
											...
											value='<c:out value="${productVO.prod_price }"/>' />
										<input style="width:40%; display: inline-block;"
											type="text" id="prod_discount" name="prod_discount" class="form-control "
											value='<c:out value="${productVO.prod_discount }"/>' />
									</div>
									<div class="form-group">
										<label for="exampleInputDetail">Detail</label>
										<textarea class="form-control" id="prod_detail" name="prod_detail" rows="8"> <c:out value="${productVO.prod_detail }"/> </textarea>
									</div>

									<div class="form-group">
										<label for="exampleInputThumbnail">Thumbnail Image</label> 
										<input type="file" id="file1" name="file1" class="form-control" />
										<input type="hidden" id="prod_img" name="prod_img" value='<c:out value="${productVO.prod_img }"/>' class="form-control" />
									</div>
									...
									<div class="form-group">
										...
										<input style="width:30%; margin-right:10px; display: inline-block;"
											...
											value='<c:out value="${productVO.prod_amount }"/>' />
										<select class="form-control" id="prod_buy" name="prod_buy" style="width: 15%; display: inline-block;">
										  <option value="Y" <c:out value="${productVO.prod_buy == 'Y' ? 'selected':'' }"/>>Y</option>
										  <option value="N" <c:out value="${productVO.prod_buy == 'N' ? 'selected':'' }"/>>N</option>
										</select>
										<!--  원래 리스트상태의 페이징, 검색정보 구성 -->
